...
describe('NotificationsService', () => {
  it('creates a notification for the user', async () => {
    ...
    const svc = new NotificationsService({ notification: { create } } as any)
  ...
  it('never throws if creation fails (best-effort)', async () => {
    const svc = new NotificationsService({ notification: { create: jest.fn().mockRejectedValue(new Error('db down')) } } as any)
  ...
  it('lists the user notifications with a client-friendly shape (lowercased kind)', async () => {
    ...
    const svc = new NotificationsService({ notification: { findMany: jest.fn().mockResolvedValue(rows) } } as any)
  ...
  it('marks all unread as read', async () => {
    ...
    const svc = new NotificationsService({ notification: { updateMany } } as any)
