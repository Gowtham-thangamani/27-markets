...
import { AgentSidebarContent } from '@/components/agent/AgentSidebar'
...
export function AgentLayout() {
  ...
  useEffect(() => {
    setMobileOpen(false)
    document.querySelector('#agent-scroll')?.scrollTo({ top: 0 })
  }, [pathname])

  return (
    <div className="flex h-dvh-safe overflow-hidden bg-ink-900">
      <a href="#agent-scroll" className="skip-link">
        Skip to content
      </a>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <>
            ...
            >
              <AgentSidebarContent onNavigate={() => setMobileOpen(false)} />
            </motion.aside>
          </>
        )}
      </AnimatePresence>
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-white/[0.06] bg-ink-900/85 px-4 backdrop-blur-xl sm:px-6">
          ...
          <div className="ml-auto flex items-center gap-2">
            <span className="flex h-10 w-10 items-center justify-center rounded-full bg-brand-500/15 text-sm font-semibold text-brand-300 ring-1 ring-brand-500/30">
              {initials(user?.name ?? 'Agent')}
            </span>
          </div>
        </header>
        <main id="agent-scroll" tabIndex={-1} className="flex-1 overflow-y-auto outline-none">
