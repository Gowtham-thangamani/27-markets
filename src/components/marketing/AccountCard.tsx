...
import type { AccountTier } from '@/mock/content'
...
export function AccountCard({ tier }: { tier: AccountTier }) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-sm text-gray-400">{tier.audience}</p>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3">
        <div className="rounded-xl border border-white/[0.06] bg-ink-800/60 p-4 text-center">
          ...
          <div className="mt-0.5 text-[11px] text-gray-500">Spreads from (pips)</div>
        </div>
        <div className="rounded-xl border border-white/[0.06] bg-ink-800/60 p-4 text-center">
          <div className="font-display text-2xl font-bold text-white">{tier.leverage}</div>
          <div className="mt-0.5 text-[11px] text-gray-500">Max leverage</div>
        </div>
      </div>
